...
export class CharacterSelectScreen implements Screen {
  ...
  mount(): void {
    ...
    this.camera.position.set(0, 0.8, 6.4);
  ...
  private buildUi(): void {
      ...
      CLASS_DEFINITIONS.map((def, i) => {
        const card = el(
          ...
          [
            el(
              'div',
              { className: 'preview', style: { display: 'flex', alignItems: 'center', justifyContent: 'center' } },
              [
                el('div', {
                  style: {
                    width: '40px',
    ...
    const screen = el(
      ...
      [
        ...
        el('div', { className: 'bottom-bar' }, [this.detailsEl, el('div', { className: 'row nav-row' }, [backBtn, confirmBtn])]),
      ],
    );
